package com.fvr.lo_location.actions;
...
import com.fvr.lo_location.db.LoAccesoBaseDatos;
import com.fvr.lo_location.forms.LoRCD_AF;
...
public class LoEDTRCD_A extends Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        LoRCD_AF pantalla = (LoRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        LoBean key = null;
        LoBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (LoBean)request.getAttribute("key_Lo");
        if ( key == null) key = new LoBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getLo_location_id() == null || key.getLo_location_id().trim().length() < 1
                ) {
		key.setLo_location_id( pantalla.getLo_location_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("location: modificar");
        ...
        if (  !( 
		   key.getLo_location_id() == null || key.getLo_location_id().trim().length() < 1
            ...
            LoAccesoBaseDatos db = new LoAccesoBaseDatos();
            try {
                registro = db.lo_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getLo_location_id() == null || pantalla.getLo_location_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            LoAccesoBaseDatos db = new LoAccesoBaseDatos();
            try {
                db.lo_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            LoAccesoBaseDatos db = new LoAccesoBaseDatos();
            try {
                db.lo_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

...
public class LoDSPFIL_A extends org.apache.struts.action.Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        cfg.setTituloPantalla("location");
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        String k;
	k = opcion; key.setLo_location_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Lo",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a location...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setLo_location_id( k );
            ///////////////////////////////////
            try {
                db.lo_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setLo_location_id( k );
            ///////////////////////////////////
            try {
                reg = db.lo_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setLo_sincro( pantalla.getLo_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setLo_mark( pantalla.getLo_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setLo_is_deleted( pantalla.getLo_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setLo_author( pantalla.getLo_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_name") ) { reg.setLo_name( pantalla.getLo_name() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_address") ) { reg.setLo_address( pantalla.getLo_address() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_city") ) { reg.setLo_city( pantalla.getLo_city() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_postal_code") ) { reg.setLo_postal_code( pantalla.getLo_postal_code() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_province") ) { reg.setLo_province( pantalla.getLo_province() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_state_region") ) { reg.setLo_state_region( pantalla.getLo_state_region() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_country") ) { reg.setLo_country( pantalla.getLo_country() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_lat") ) { reg.setLo_lat( pantalla.getLo_lat() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_lon") ) { reg.setLo_lon( pantalla.getLo_lon() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_comment") ) { reg.setLo_comment( pantalla.getLo_comment() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setLo_json( pantalla.getLo_json() ); }
                    ///////////////////////////////////
                    db.lo_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("loFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("loFilaInicioGrid")).intValue() );
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        LoBean rcd_Lo = (LoBean) request.getAttribute("rcd_Lo");
        if ( rcd_Lo != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getLo_rst0() == null || pantalla.getLo_rst0().trim().length() < 1 ) {
            //     pantalla.setLo_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
            ...
            for (int i=0;i<pantalla.getGrid().length;i++) {
                ...
                if ( key.getChg() != null && key.getChg().trim().length() > 0 ) {
                    try {
                        ...
                        String k;
	k = opcion; key.setLo_location_id( k );
                        reg = db.lo_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setLo_Campo( ((LoBean)(pantalla.getGrid()[i])).getLo_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.lo_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

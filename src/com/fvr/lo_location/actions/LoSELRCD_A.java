...
public class LoSELRCD_A extends org.apache.struts.action.Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm form) {
        ...
        try {
            pantalla.setGrid( db.lo_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getLo_filtro()
    ...
    private void inicializarPantalla(HttpServletRequest request, ActionForm form) {
        ...
        LoBeanFiltro filtroOld = (LoBeanFiltro) request.getSession().getAttribute("loFiltro");
        if ( filtroOld != null ) { pantalla.setLo_filtro( (LoBeanFiltro) filtroOld.coalesce(pantalla.getLo_filtro(),filtroOld) ); }

        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("loFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("loFiltro",pantalla.getLo_filtro());

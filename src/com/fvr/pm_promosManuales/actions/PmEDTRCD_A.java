...
public class PmEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        PmBean key = null;
        PmBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (PmBean)request.getAttribute("key_Pm");
        if ( key == null) key = new PmBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getPm_coupon_id() == null || key.getPm_coupon_id().trim().length() < 1
                ) {
		key.setPm_coupon_id( pantalla.getPm_coupon_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("promosManuales: modificar");
        ...
        if (  !( 
		   key.getPm_coupon_id() == null || key.getPm_coupon_id().trim().length() < 1
            ...
            try {
                registro = db.pm_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getPm_coupon_id() == null || pantalla.getPm_coupon_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            PmAccesoBaseDatos db = new PmAccesoBaseDatos();
            try {
                db.pm_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            PmAccesoBaseDatos db = new PmAccesoBaseDatos();
            try {
                db.pm_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

package com.fvr.ps_countries.actions;
...
import com.fvr.ps_countries.db.PsAccesoBaseDatos;
import com.fvr.ps_countries.forms.PsRCD_AF;
...
public class PsDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        PsRCD_AF pantalla = (PsRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        PsAccesoBaseDatos db = new PsAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("countries");
        ...
        try {
            ...
            pantalla.setGrid( db.ps_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getPs_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                PsBean reg = (PsBean) pantalla.getGrid()[i];
                // Guardar la CLAVE ÚNICA DEL SUBARCHIVO !!!
                pantalla.getClavesMarcadas()[i] = 
		new Long(reg.getPs_country_id()).toString();
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        String k;
	k = opcion; key.setPs_country_id( new Long(k).longValue() );
        ///////////////////////////////////
        request.setAttribute("key_Ps",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a countries...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setPs_country_id( new Long(k).longValue() );
            ///////////////////////////////////
            try {
                db.ps_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setPs_country_id( new Long(k).longValue() );
            ///////////////////////////////////
            try {
                reg = db.ps_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setPs_sincro( pantalla.getPs_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setPs_mark( pantalla.getPs_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setPs_is_deleted( pantalla.getPs_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setPs_author( pantalla.getPs_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_name") ) { reg.setPs_name( pantalla.getPs_name() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_alpha_2") ) { reg.setPs_alpha_2( pantalla.getPs_alpha_2() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_alpha_3") ) { reg.setPs_alpha_3( pantalla.getPs_alpha_3() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_flag_base64") ) { reg.setPs_flag_base64( pantalla.getPs_flag_base64() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setPs_json( pantalla.getPs_json() ); }
                    ///////////////////////////////////
                    db.ps_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("psFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("psFilaInicioGrid")).intValue() );
        ...
        PsBeanFiltro filtroOld = (PsBeanFiltro) request.getSession().getAttribute("psFiltro");
        if ( filtroOld != null ) { pantalla.setPs_filtro( (PsBeanFiltro) filtroOld.coalesce(pantalla.getPs_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("psFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("psFiltro",pantalla.getPs_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        PsBean rcd_Ps = (PsBean) request.getAttribute("rcd_Ps");
        if ( rcd_Ps != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getPs_rst0() == null || pantalla.getPs_rst0().trim().length() < 1 ) {
            //     pantalla.setPs_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
            ...
            for (int i=0;i<pantalla.getGrid().length;i++) {
                ...
                if ( key.getChg() != null && key.getChg().trim().length() > 0 ) {
                    try {
                        ...
                        String k;
	k = opcion; key.setPs_country_id( new Long(k).longValue() );
                        reg = db.ps_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setPs_Campo( ((PsBean)(pantalla.getGrid()[i])).getPs_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.ps_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

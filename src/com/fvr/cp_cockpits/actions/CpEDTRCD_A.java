...
public class CpEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        CpBean key = null;
        CpBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (CpBean)request.getAttribute("key_Cp");
        if ( key == null) key = new CpBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getCp_cockpit_id() == null || key.getCp_cockpit_id().trim().length() < 1
		|| key.getCp_location_id() == null || key.getCp_location_id().trim().length() < 1
                ) {
		key.setCp_cockpit_id( pantalla.getCp_cockpit_id() );
		key.setCp_location_id( pantalla.getCp_location_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("cockpits: modificar");
		...
		|| key.getCp_location_id() == null || key.getCp_location_id().trim().length() < 1
            ...
            try {
                registro = db.cp_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getCp_cockpit_id() == null || pantalla.getCp_cockpit_id().trim().length() < 1
		|| pantalla.getCp_location_id() == null || pantalla.getCp_location_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            CpAccesoBaseDatos db = new CpAccesoBaseDatos();
            try {
                db.cp_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            CpAccesoBaseDatos db = new CpAccesoBaseDatos();
            try {
                db.cp_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

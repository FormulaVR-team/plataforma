package com.fvr.cp_cockpits.actions;
...
import com.fvr.cp_cockpits.db.CpAccesoBaseDatos;
import com.fvr.cp_cockpits.forms.CpRCD_AF;
...
public class CpDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        CpRCD_AF pantalla = (CpRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        CpAccesoBaseDatos db = new CpAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("cockpits");
        ...
        try {
            ...
            pantalla.setGrid( db.cp_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getCp_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                CpBean reg = (CpBean) pantalla.getGrid()[i];
                // Guardar la CLAVE ÚNICA DEL SUBARCHIVO !!!
                pantalla.getClavesMarcadas()[i] = 
		reg.getCp_cockpit_id() + "^" + 
		reg.getCp_location_id();
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        CpBean key = new CpBean();
        String k;
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setCp_cockpit_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setCp_location_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Cp",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a cockpits...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setCp_location_id( k );
            ///////////////////////////////////
            try {
                db.cp_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setCp_location_id( k );
            ///////////////////////////////////
            try {
                reg = db.cp_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setCp_sincro( pantalla.getCp_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setCp_mark( pantalla.getCp_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setCp_is_deleted( pantalla.getCp_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setCp_author( pantalla.getCp_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_serial_number") ) { reg.setCp_serial_number( pantalla.getCp_serial_number() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_name") ) { reg.setCp_name( pantalla.getCp_name() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_isBlocked") ) { reg.setCp_isBlocked( pantalla.getCp_isBlocked() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_asignation_order") ) { reg.setCp_asignation_order( pantalla.getCp_asignation_order() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_install_date") ) { reg.setCp_install_date( pantalla.getCp_install_date() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_reset_date_used") ) { reg.setCp_reset_date_used( pantalla.getCp_reset_date_used() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_hours_used") ) { reg.setCp_hours_used( pantalla.getCp_hours_used() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_note") ) { reg.setCp_note( pantalla.getCp_note() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_comment") ) { reg.setCp_comment( pantalla.getCp_comment() ); }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("cpFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("cpFilaInicioGrid")).intValue() );
        ...
        CpBeanFiltro filtroOld = (CpBeanFiltro) request.getSession().getAttribute("cpFiltro");
        if ( filtroOld != null ) { pantalla.setCp_filtro( (CpBeanFiltro) filtroOld.coalesce(pantalla.getCp_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("cpFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("cpFiltro",pantalla.getCp_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        CpBean rcd_Cp = (CpBean) request.getAttribute("rcd_Cp");
        if ( rcd_Cp != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getCp_rst0() == null || pantalla.getCp_rst0().trim().length() < 1 ) {
            //     pantalla.setCp_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
	...
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setCp_cockpit_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setCp_location_id( k );
                        reg = db.cp_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setCp_Campo( ((CpBean)(pantalla.getGrid()[i])).getCp_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.cp_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

package com.fvr.tk_tokens.actions;
...
import com.fvr.tk_tokens.db.TkAccesoBaseDatos;
import com.fvr.tk_tokens.forms.TkRCD_AF;
...
public class TkDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        TkRCD_AF pantalla = (TkRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        TkAccesoBaseDatos db = new TkAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        try {
            ...
            pantalla.setGrid( db.tk_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getTk_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                TkBean reg = (TkBean) pantalla.getGrid()[i];
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        String k;
	k = opcion; key.setTk_token_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Tk",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a tokens...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setTk_token_id( k );
            ///////////////////////////////////
            try {
                db.tk_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setTk_token_id( k );
            ///////////////////////////////////
            try {
                reg = db.tk_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setTk_sincro( pantalla.getTk_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setTk_mark( pantalla.getTk_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setTk_is_deleted( pantalla.getTk_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setTk_author( pantalla.getTk_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setTk_json( pantalla.getTk_json() ); }
                    ///////////////////////////////////
                    db.tk_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("tkFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("tkFilaInicioGrid")).intValue() );
        ...
        TkBeanFiltro filtroOld = (TkBeanFiltro) request.getSession().getAttribute("tkFiltro");
        if ( filtroOld != null ) { pantalla.setTk_filtro( (TkBeanFiltro) filtroOld.coalesce(pantalla.getTk_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("tkFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("tkFiltro",pantalla.getTk_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        TkBean rcd_Tk = (TkBean) request.getAttribute("rcd_Tk");
        if ( rcd_Tk != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getTk_rst0() == null || pantalla.getTk_rst0().trim().length() < 1 ) {
            //     pantalla.setTk_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
            ...
            for (int i=0;i<pantalla.getGrid().length;i++) {
                ...
                if ( key.getChg() != null && key.getChg().trim().length() > 0 ) {
                    try {
                        ...
                        String k;
	k = opcion; key.setTk_token_id( k );
                        reg = db.tk_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setTk_Campo( ((TkBean)(pantalla.getGrid()[i])).getTk_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.tk_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

...
public class TkEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        TkBean key = null;
        TkBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (TkBean)request.getAttribute("key_Tk");
        if ( key == null) key = new TkBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getTk_token_id() == null || key.getTk_token_id().trim().length() < 1
                ) {
		key.setTk_token_id( pantalla.getTk_token_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("tokens: modificar");
        ...
        if (  !( 
		   key.getTk_token_id() == null || key.getTk_token_id().trim().length() < 1
            ...
            try {
                registro = db.tk_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getTk_token_id() == null || pantalla.getTk_token_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            TkAccesoBaseDatos db = new TkAccesoBaseDatos();
            try {
                db.tk_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            TkAccesoBaseDatos db = new TkAccesoBaseDatos();
            try {
                db.tk_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

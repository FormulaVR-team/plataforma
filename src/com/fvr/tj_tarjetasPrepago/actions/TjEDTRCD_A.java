...
public class TjEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        key = (TjBean)request.getAttribute("key_Tj");
        if ( key == null) key = new TjBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getTj_card_id() == null || key.getTj_card_id().trim().length() < 1
                ) {
		key.setTj_card_id( pantalla.getTj_card_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("tarjetasPrepago: modificar");
        ...
        if (  !( 
		   key.getTj_card_id() == null || key.getTj_card_id().trim().length() < 1
            ...
            try {
                registro = db.tj_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            TjAccesoBaseDatos db = new TjAccesoBaseDatos();
            try {
                db.tj_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            TjAccesoBaseDatos db = new TjAccesoBaseDatos();
            try {
                db.tj_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

...
public class TjBeanFiltro extends StBean {
    ...
    public TjBeanFiltro coalesce(TjBeanFiltro pri, TjBeanFiltro sec) {
		...
		r.setTj_user_id( (pri.getTj_user_id()==null)?sec.getTj_user_id():pri.getTj_user_id() );	// user_id
	...
	public void deserializar(String in) {
		...
		if ( in != null && in.length() > 0 ) {
			...
			try { this.setTj_balance_initial( trozos[6] ); } catch (Exception e) {;} // balance_initial
			try { this.setTj_balance_current( trozos[7] ); } catch (Exception e) {;} // balance_current
			try { this.setTj_last_sale_amount( trozos[8] ); } catch (Exception e) {;} // last_sale_amount

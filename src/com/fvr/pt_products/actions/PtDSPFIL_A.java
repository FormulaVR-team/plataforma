package com.fvr.pt_products.actions;
...
import com.fvr.pt_products.db.PtAccesoBaseDatos;
import com.fvr.pt_products.forms.PtRCD_AF;
...
public class PtDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        PtRCD_AF pantalla = (PtRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        PtAccesoBaseDatos db = new PtAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("products");
        ...
        try {
            ...
            pantalla.setGrid( db.pt_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getPt_filtro()
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        String k;
	k = opcion; key.setPt_product_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Pt",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a products...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setPt_product_id( k );
            ///////////////////////////////////
            try {
                db.pt_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setPt_product_id( k );
            ///////////////////////////////////
            try {
                reg = db.pt_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setPt_sincro( pantalla.getPt_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setPt_mark( pantalla.getPt_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setPt_is_deleted( pantalla.getPt_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setPt_author( pantalla.getPt_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_name") ) { reg.setPt_name( pantalla.getPt_name() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_whoCanSelect_AFU") ) { reg.setPt_whoCanSelect_AFU( pantalla.getPt_whoCanSelect_AFU() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_deadline") ) { reg.setPt_deadline( pantalla.getPt_deadline() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_isPercent") ) { reg.setPt_isPercent( pantalla.getPt_isPercent() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_amount") ) { reg.setPt_amount( pantalla.getPt_amount() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_currency") ) { reg.setPt_currency( pantalla.getPt_currency() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_duration_minutes") ) { reg.setPt_duration_minutes( pantalla.getPt_duration_minutes() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setPt_json( pantalla.getPt_json() ); }
                    ///////////////////////////////////
                    db.pt_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("ptFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("ptFilaInicioGrid")).intValue() );
        ...
        PtBeanFiltro filtroOld = (PtBeanFiltro) request.getSession().getAttribute("ptFiltro");
        if ( filtroOld != null ) { pantalla.setPt_filtro( (PtBeanFiltro) filtroOld.coalesce(pantalla.getPt_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("ptFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("ptFiltro",pantalla.getPt_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        PtBean rcd_Pt = (PtBean) request.getAttribute("rcd_Pt");
        if ( rcd_Pt != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getPt_rst0() == null || pantalla.getPt_rst0().trim().length() < 1 ) {
            //     pantalla.setPt_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
            ...
            for (int i=0;i<pantalla.getGrid().length;i++) {
                ...
                if ( key.getChg() != null && key.getChg().trim().length() > 0 ) {
                    try {
                        ...
                        String k;
	k = opcion; key.setPt_product_id( k );
                        reg = db.pt_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setPt_Campo( ((PtBean)(pantalla.getGrid()[i])).getPt_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.pt_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

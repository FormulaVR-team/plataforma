...
public class PtEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        PtBean key = null;
        PtBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (PtBean)request.getAttribute("key_Pt");
        if ( key == null) key = new PtBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getPt_product_id() == null || key.getPt_product_id().trim().length() < 1
                ) {
		key.setPt_product_id( pantalla.getPt_product_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("products: modificar");
        ...
        if (  !( 
		   key.getPt_product_id() == null || key.getPt_product_id().trim().length() < 1
            ...
            try {
                registro = db.pt_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getPt_product_id() == null || pantalla.getPt_product_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            PtAccesoBaseDatos db = new PtAccesoBaseDatos();
            try {
                db.pt_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            PtAccesoBaseDatos db = new PtAccesoBaseDatos();
            try {
                db.pt_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

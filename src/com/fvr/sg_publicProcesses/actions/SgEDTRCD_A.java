...
public class SgEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        SgBean key = null;
        SgBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (SgBean)request.getAttribute("key_Sg");
        if ( key == null) key = new SgBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getSg_role_id() == null || key.getSg_role_id().trim().length() < 1
		|| key.getSg_process_id() == null || key.getSg_process_id().trim().length() < 1
                ) {
		key.setSg_role_id( pantalla.getSg_role_id() );
		key.setSg_process_id( pantalla.getSg_process_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("publicProcesses: modificar");
		...
		|| key.getSg_process_id() == null || key.getSg_process_id().trim().length() < 1
            ...
            try {
                registro = db.sg_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getSg_role_id() == null || pantalla.getSg_role_id().trim().length() < 1
		|| pantalla.getSg_process_id() == null || pantalla.getSg_process_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            SgAccesoBaseDatos db = new SgAccesoBaseDatos();
            try {
                db.sg_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            SgAccesoBaseDatos db = new SgAccesoBaseDatos();
            try {
                db.sg_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

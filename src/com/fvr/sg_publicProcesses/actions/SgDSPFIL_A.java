package com.fvr.sg_publicProcesses.actions;
...
import com.fvr.sg_publicProcesses.db.SgAccesoBaseDatos;
import com.fvr.sg_publicProcesses.forms.SgRCD_AF;
...
public class SgDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        SgRCD_AF pantalla = (SgRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        SgAccesoBaseDatos db = new SgAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("publicProcesses");
        ...
        try {
            ...
            pantalla.setGrid( db.sg_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getSg_filtro()
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        SgBean key = new SgBean();
        String k;
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setSg_role_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setSg_process_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Sg",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a publicProcesses...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setSg_process_id( k );
            ///////////////////////////////////
            try {
                db.sg_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setSg_process_id( k );
            ///////////////////////////////////
            try {
                reg = db.sg_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setSg_sincro( pantalla.getSg_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setSg_mark( pantalla.getSg_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setSg_is_deleted( pantalla.getSg_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setSg_author( pantalla.getSg_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setSg_json( pantalla.getSg_json() ); }
                    ///////////////////////////////////
                    db.sg_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("sgFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("sgFilaInicioGrid")).intValue() );
        ...
        SgBeanFiltro filtroOld = (SgBeanFiltro) request.getSession().getAttribute("sgFiltro");
        if ( filtroOld != null ) { pantalla.setSg_filtro( (SgBeanFiltro) filtroOld.coalesce(pantalla.getSg_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("sgFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("sgFiltro",pantalla.getSg_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        SgBean rcd_Sg = (SgBean) request.getAttribute("rcd_Sg");
        if ( rcd_Sg != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getSg_rst0() == null || pantalla.getSg_rst0().trim().length() < 1 ) {
            //     pantalla.setSg_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
	...
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setSg_role_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setSg_process_id( k );
                        reg = db.sg_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setSg_Campo( ((SgBean)(pantalla.getGrid()[i])).getSg_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.sg_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

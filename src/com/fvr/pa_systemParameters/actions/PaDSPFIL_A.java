...
public class PaDSPFIL_A extends org.apache.struts.action.Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        cfg.setTituloPantalla("systemParameters");
        ...
        try {
            ...
            pantalla.setGrid( db.pa_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getPa_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                PaBean reg = (PaBean) pantalla.getGrid()[i];
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        PaBean key = new PaBean();
        String k;
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setPa_group_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setPa_key( k );
        ///////////////////////////////////
        request.setAttribute("key_Pa",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a systemParameters...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setPa_key( k );
            ///////////////////////////////////
            try {
                db.pa_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setPa_key( k );
            ///////////////////////////////////
            try {
                reg = db.pa_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setPa_sincro( pantalla.getPa_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setPa_mark( pantalla.getPa_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setPa_is_deleted( pantalla.getPa_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setPa_author( pantalla.getPa_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_value") ) { reg.setPa_value( pantalla.getPa_value() ); }
                    ///////////////////////////////////
                    db.pa_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("paFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("paFilaInicioGrid")).intValue() );
        ...
        PaBeanFiltro filtroOld = (PaBeanFiltro) request.getSession().getAttribute("paFiltro");
        if ( filtroOld != null ) { pantalla.setPa_filtro( (PaBeanFiltro) filtroOld.coalesce(pantalla.getPa_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("paFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("paFiltro",pantalla.getPa_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        PaBean rcd_Pa = (PaBean) request.getAttribute("rcd_Pa");
        if ( rcd_Pa != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getPa_rst0() == null || pantalla.getPa_rst0().trim().length() < 1 ) {
            //     pantalla.setPa_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
	...
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setPa_group_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setPa_key( k );
                        reg = db.pa_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setPa_Campo( ((PaBean)(pantalla.getGrid()[i])).getPa_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.pa_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

...
public class LpEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        LpBean key = null;
        LpBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (LpBean)request.getAttribute("key_Lp");
        if ( key == null) key = new LpBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getLp_card_id() == null || key.getLp_card_id().trim().length() < 1
                ) {
		key.setLp_card_id( pantalla.getLp_card_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("labels2print: modificar");
        ...
        if (  !( 
		   key.getLp_card_id() == null || key.getLp_card_id().trim().length() < 1
            ...
            try {
                registro = db.lp_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getLp_card_id() == null || pantalla.getLp_card_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            LpAccesoBaseDatos db = new LpAccesoBaseDatos();
            try {
                db.lp_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            LpAccesoBaseDatos db = new LpAccesoBaseDatos();
            try {
                db.lp_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

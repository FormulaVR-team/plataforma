...
public class CdEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        CdBean key = null;
        CdBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (CdBean)request.getAttribute("key_Cd");
        if ( key == null) key = new CdBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getCd_location_id() == null || key.getCd_location_id().trim().length() < 1
		|| key.getCd_closed_day_aaaa_mm_dd() == null || key.getCd_closed_day_aaaa_mm_dd().trim().length() < 1
                ) {
		key.setCd_location_id( pantalla.getCd_location_id() );
		key.setCd_closed_day_aaaa_mm_dd( pantalla.getCd_closed_day_aaaa_mm_dd() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("LocationClosedDays: modificar");
		...
		|| key.getCd_closed_day_aaaa_mm_dd() == null || key.getCd_closed_day_aaaa_mm_dd().trim().length() < 1
            ...
            try {
                registro = db.cd_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getCd_location_id() == null || pantalla.getCd_location_id().trim().length() < 1
		|| pantalla.getCd_closed_day_aaaa_mm_dd() == null || pantalla.getCd_closed_day_aaaa_mm_dd().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            CdAccesoBaseDatos db = new CdAccesoBaseDatos();
            try {
                db.cd_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            CdAccesoBaseDatos db = new CdAccesoBaseDatos();
            try {
                db.cd_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

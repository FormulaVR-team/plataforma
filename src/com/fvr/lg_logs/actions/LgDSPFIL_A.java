package com.fvr.lg_logs.actions;
...
import com.fvr.lg_logs.db.LgAccesoBaseDatos;
import com.fvr.lg_logs.forms.LgRCD_AF;
...
public class LgDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        LgRCD_AF pantalla = (LgRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        LgAccesoBaseDatos db = new LgAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        try {
            ...
            pantalla.setGrid( db.lg_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getLg_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                LgBean reg = (LgBean) pantalla.getGrid()[i];
                // Guardar la CLAVE ÚNICA DEL SUBARCHIVO !!!
                pantalla.getClavesMarcadas()[i] = 
		new Long(reg.getLg_serial()).toString();
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        LgBean key = new LgBean();
        String k;
	k = opcion; key.setLg_serial( new Long(k).longValue() );
        ///////////////////////////////////
        request.setAttribute("key_Lg",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a logs...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setLg_serial( new Long(k).longValue() );
            ///////////////////////////////////
            try {
                db.lg_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setLg_serial( new Long(k).longValue() );
            ///////////////////////////////////
            try {
                reg = db.lg_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setLg_sincro( pantalla.getLg_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setLg_mark( pantalla.getLg_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setLg_is_deleted( pantalla.getLg_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setLg_author( pantalla.getLg_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_text_1") ) { reg.setLg_text_1( pantalla.getLg_text_1() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_text_2") ) { reg.setLg_text_2( pantalla.getLg_text_2() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_text_3") ) { reg.setLg_text_3( pantalla.getLg_text_3() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setLg_json( pantalla.getLg_json() ); }
                    ///////////////////////////////////
                    db.lg_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("lgFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("lgFilaInicioGrid")).intValue() );
        ...
        LgBeanFiltro filtroOld = (LgBeanFiltro) request.getSession().getAttribute("lgFiltro");
        if ( filtroOld != null ) { pantalla.setLg_filtro( (LgBeanFiltro) filtroOld.coalesce(pantalla.getLg_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("lgFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("lgFiltro",pantalla.getLg_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        LgBean rcd_Lg = (LgBean) request.getAttribute("rcd_Lg");
        if ( rcd_Lg != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getLg_rst0() == null || pantalla.getLg_rst0().trim().length() < 1 ) {
            //     pantalla.setLg_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
            ...
            for (int i=0;i<pantalla.getGrid().length;i++) {
                ...
                if ( key.getChg() != null && key.getChg().trim().length() > 0 ) {
                    try {
                        ...
                        String k;
	k = opcion; key.setLg_serial( new Long(k).longValue() );
                        reg = db.lg_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setLg_Campo( ((LgBean)(pantalla.getGrid()[i])).getLg_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.lg_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

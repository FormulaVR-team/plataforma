...
public class TsDSPFIL_A extends org.apache.struts.action.Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        cfg.setTituloPantalla("timeSlices");
        ...
        try {
            ...
            pantalla.setGrid( db.ts_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getTs_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                TsBean reg = (TsBean) pantalla.getGrid()[i];
                // Guardar la CLAVE ÚNICA DEL SUBARCHIVO !!!
                pantalla.getClavesMarcadas()[i] = 
		reg.getTs_reservation_id() + "^" + 
		reg.getTs_start_date() + "^" + 
		reg.getTs_start_time();
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        TsBean key = new TsBean();
        String k;
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setTs_reservation_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setTs_start_date( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setTs_start_time( k );
        ///////////////////////////////////
        request.setAttribute("key_Ts",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a timeSlices...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setTs_start_time( k );
            ///////////////////////////////////
            try {
                db.ts_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setTs_start_time( k );
            ///////////////////////////////////
            try {
                reg = db.ts_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setTs_sincro( pantalla.getTs_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setTs_mark( pantalla.getTs_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setTs_is_deleted( pantalla.getTs_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setTs_author( pantalla.getTs_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setTs_json( pantalla.getTs_json() ); }
                    ///////////////////////////////////
                    db.ts_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("tsFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("tsFilaInicioGrid")).intValue() );
        ...
        TsBeanFiltro filtroOld = (TsBeanFiltro) request.getSession().getAttribute("tsFiltro");
        if ( filtroOld != null ) { pantalla.setTs_filtro( (TsBeanFiltro) filtroOld.coalesce(pantalla.getTs_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("tsFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("tsFiltro",pantalla.getTs_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        TsBean rcd_Ts = (TsBean) request.getAttribute("rcd_Ts");
        if ( rcd_Ts != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getTs_rst0() == null || pantalla.getTs_rst0().trim().length() < 1 ) {
            //     pantalla.setTs_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
	...
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setTs_start_date( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setTs_start_time( k );
                        reg = db.ts_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setTs_Campo( ((TsBean)(pantalla.getGrid()[i])).getTs_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.ts_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

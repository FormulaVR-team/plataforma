...
public class TsBean extends StBean {
    ...
    public TsBean(Object nulo) { super(); }
	...
	this.setTs_RS_quantity( 0 ); // RS_quantity
	this.setTs_RS_duration_minutes( 0 ); // RS_duration_minutes
	this.setTs_RS_places( 0 ); // RS_places
...
*/
    ...
    public String getKey(){
		 return ts_reservation_id + "^" + 
		ts_start_date + "^" + 
		ts_start_time;}

    public void setKey(String key){
            String k="";
	k = key.trim().substring( 0, key.indexOf("^") ); this.setTs_reservation_id( k ); key = key.trim().substring( key.indexOf("^")+1 );
	k = key.trim().substring( 0, key.indexOf("^") ); this.setTs_start_date( k ); key = key.trim().substring( key.indexOf("^")+1 );
	k = key; this.setTs_start_time( k );
    }
	////////////////////////////////////////////////////////////
	public String serializar() {
		...
		out.append( _K.sepFld ); out.append( this.getTs_RS_quantity() ); // RS_quantity
		out.append( _K.sepFld ); out.append( this.getTs_RS_duration_minutes() ); // RS_duration_minutes
		out.append( _K.sepFld ); out.append( this.getTs_RS_places() ); // RS_places
	...
	public void deserializar(String in) {
		...
		if ( in != null && in.length() > 0 ) {
			...
			try { this.setTs_RS_quantity( Subrutinas.parse_long( trozos[11] )); } catch (Exception e) {;} // RS_quantity
			try { this.setTs_RS_duration_minutes( Subrutinas.parse_long( trozos[12] )); } catch (Exception e) {;} // RS_duration_minutes
			try { this.setTs_RS_places( Subrutinas.parse_long( trozos[13] )); } catch (Exception e) {;} // RS_places

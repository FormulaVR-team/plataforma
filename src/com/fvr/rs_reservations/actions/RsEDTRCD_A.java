...
public class RsEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        key = (RsBean)request.getAttribute("key_Rs");
        if ( key == null) key = new RsBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getRs_reservation_id() == null || key.getRs_reservation_id().trim().length() < 1
                ) {
		key.setRs_reservation_id( pantalla.getRs_reservation_id() );
            }
        ///////////////////////////////////////////
        ...
        if (  !( 
		   key.getRs_reservation_id() == null || key.getRs_reservation_id().trim().length() < 1
            ...
            try {
                registro = db.rs_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            RsAccesoBaseDatos db = new RsAccesoBaseDatos();
            try {
                db.rs_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            RsAccesoBaseDatos db = new RsAccesoBaseDatos();
            try {
                db.rs_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

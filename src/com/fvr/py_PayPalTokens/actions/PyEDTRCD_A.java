...
public class PyEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        PyBean key = null;
        PyBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (PyBean)request.getAttribute("key_Py");
        if ( key == null) key = new PyBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getPy_user_id() == null || key.getPy_user_id().trim().length() < 1
		|| key.getPy_reservation_id() == null || key.getPy_reservation_id().trim().length() < 1
		|| key.getPy_paypal_token_id() == null || key.getPy_paypal_token_id().trim().length() < 1
                ) {
		key.setPy_user_id( pantalla.getPy_user_id() );
		key.setPy_reservation_id( pantalla.getPy_reservation_id() );
		key.setPy_paypal_token_id( pantalla.getPy_paypal_token_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("PayPalTokens: modificar");
		...
		|| key.getPy_paypal_token_id() == null || key.getPy_paypal_token_id().trim().length() < 1
            ...
            try {
                registro = db.py_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getPy_user_id() == null || pantalla.getPy_user_id().trim().length() < 1
		|| pantalla.getPy_reservation_id() == null || pantalla.getPy_reservation_id().trim().length() < 1
		|| pantalla.getPy_paypal_token_id() == null || pantalla.getPy_paypal_token_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            PyAccesoBaseDatos db = new PyAccesoBaseDatos();
            try {
                db.py_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            PyAccesoBaseDatos db = new PyAccesoBaseDatos();
            try {
                db.py_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

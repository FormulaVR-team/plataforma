package com.fvr.py_PayPalTokens.actions;
...
import com.fvr.py_PayPalTokens.forms.PyRCD_AF;
...
public class PyDSPFIL_A extends org.apache.struts.action.Action {
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm  form,
        ...
        PyRCD_AF pantalla = (PyRCD_AF)form;
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        ///////////////////////////////////////////
        PyAccesoBaseDatos db = new PyAccesoBaseDatos();
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("PayPalTokens");
        ...
        try {
            ...
            pantalla.setGrid( db.py_getSeq(
                    new Subrutinas().getBDConexion(request)
                    , cfg
                    , pantalla.getPy_filtro()
    ...
    private String opcion_Selec_Marcar( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( pantalla.getGrid() != null  ) {
            ...
            for (int i=0;i<pantalla.getGrid().length; i++) {
                PyBean reg = (PyBean) pantalla.getGrid()[i];
                // Guardar la CLAVE ÚNICA DEL SUBARCHIVO !!!
                pantalla.getClavesMarcadas()[i] = 
		reg.getPy_user_id() + "^" + 
		reg.getPy_reservation_id() + "^" + 
		reg.getPy_paypal_token_id();
    ...
    private String opcion_Edit( HttpServletRequest request, ActionForm  form ) {
        ...
        PyBean key = new PyBean();
        String k;
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setPy_user_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setPy_reservation_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setPy_paypal_token_id( k );
        ///////////////////////////////////
        request.setAttribute("key_Py",key);
        ///////////////////////////////////
        resultado = "EDTRCD";
        ///////////////////////////////////
        return resultado;
    ...
    private String opcion_Exportar( HttpServletRequest request, ActionForm  form ) {
        ...
        if (resultado.equalsIgnoreCase("OK")) {
            ...
            // Enlace al resultado:
            ActionMessages errores = new ActionMessages();
            errores.add("error", new ActionMessage( "errors.detail", "<a href='" + NombreArchivo + "' target='pepito'>Enlace a PayPalTokens...</a>" ));
            saveErrors(request,errores);
	...
	k = opcion; key.setPy_paypal_token_id( k );
            ///////////////////////////////////
            try {
                db.py_dltObj( new Subrutinas().getBDConexion(request), key );
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
            //////////////////////////////
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
	...
	k = opcion; key.setPy_paypal_token_id( k );
            ///////////////////////////////////
            try {
                reg = db.py_getRcd( database, key );
                if ( reg != null) {
                    ///////////////////////////////////
                    // Aplico el nuevo valor al registro:
		if ( accion.trim().equalsIgnoreCase("colectivo_sincro") ) { reg.setPy_sincro( pantalla.getPy_sincro() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_mark") ) { reg.setPy_mark( pantalla.getPy_mark() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_is_deleted") ) { reg.setPy_is_deleted( pantalla.getPy_is_deleted() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_author") ) { reg.setPy_author( pantalla.getPy_author() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_paypal_usr") ) { reg.setPy_paypal_usr( pantalla.getPy_paypal_usr() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_paypal_pwd") ) { reg.setPy_paypal_pwd( pantalla.getPy_paypal_pwd() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_paypal_signature") ) { reg.setPy_paypal_signature( pantalla.getPy_paypal_signature() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_stsProceso") ) { reg.setPy_stsProceso( pantalla.getPy_stsProceso() ); }
		if ( accion.trim().equalsIgnoreCase("colectivo_json") ) { reg.setPy_json( pantalla.getPy_json() ); }
                    ///////////////////////////////////
                    db.py_chgObj( database, reg );
                }
            } catch (StExcepcion ex) {
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
            }
...
//        errores.add("error", new ActionMessage( "errors.detail", nfilas + " registros procesados." ));
    ...
    private void inicializarPantalla( HttpServletRequest request, ActionForm  form ) {
        ...
        if ( request.getSession().getAttribute("pyFilaInicioGrid") != null && pantalla.getFilaInicioGrid() == 0 )
            pantalla.setFilaInicioGrid( ((java.lang.Integer) request.getSession().getAttribute("pyFilaInicioGrid")).intValue() );
        ...
        PyBeanFiltro filtroOld = (PyBeanFiltro) request.getSession().getAttribute("pyFiltro");
        if ( filtroOld != null ) { pantalla.setPy_filtro( (PyBeanFiltro) filtroOld.coalesce(pantalla.getPy_filtro(),filtroOld) ); }
        
        // Inicializar filtros:
        
        // PUSH SuperFiltros en SESSION:
        request.getSession().setAttribute("pyFilaInicioGrid", new java.lang.Integer( pantalla.getFilaInicioGrid()) );
        request.getSession().setAttribute("FilasGrid", new java.lang.Integer( pantalla.getFilasGrid()) );
        
        // PUSH Filtros en SESSION:
        request.getSession().setAttribute("pyFiltro",pantalla.getPy_filtro());
    ...
    private void aplicarParametrosDeEntrada(HttpServletRequest request, ActionForm form ) {
        ...
        PyBean rcd_Py = (PyBean) request.getAttribute("rcd_Py");
        if ( rcd_Py != null ) {
            // Ha sido llamado desde otro programa:
            ;
        } else {
            // Recarga de sí misma...verificar si valores legales en campos importantes u obligatorios:
            // if ( pantalla.getPy_rst0() == null || pantalla.getPy_rst0().trim().length() < 1 ) {
            //     pantalla.setPy_rst0( "ValorDeProteccion" );
            // }
            ;
        }
        ///////////////////////////////////////////
	...
	k = opcion.trim().substring( 0, opcion.indexOf("^") ); key.setPy_reservation_id( k ); opcion = opcion.trim().substring( opcion.indexOf("^")+1 );
	k = opcion; key.setPy_paypal_token_id( k );
                        reg = db.py_getRcd( new Subrutinas().getBDConexion(request), key );
                        if (reg!=null) {
                            // Campo(s) a reescribir:
                            // reg.setPy_Campo( ((PyBean)(pantalla.getGrid()[i])).getPy_Campo() );
                            // ...meter a manita cada campo...y activar este chgObj:
                            // db.py_chgObj(new Subrutinas().getBDConexion(request),reg);
                            resultado++;
                            }
                    } catch (StExcepcion ex) {;}
                }

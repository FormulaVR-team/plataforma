...
public class PrEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        PrBean key = null;
        PrBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (PrBean)request.getAttribute("key_Pr");
        if ( key == null) key = new PrBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getPr_location_id() == null || key.getPr_location_id().trim().length() < 1
		|| key.getPr_product_id() == null || key.getPr_product_id().trim().length() < 1
                ) {
		key.setPr_location_id( pantalla.getPr_location_id() );
		key.setPr_product_id( pantalla.getPr_product_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("promos: modificar");
		...
		|| key.getPr_product_id() == null || key.getPr_product_id().trim().length() < 1
            ...
            try {
                registro = db.pr_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getPr_location_id() == null || pantalla.getPr_location_id().trim().length() < 1
		|| pantalla.getPr_product_id() == null || pantalla.getPr_product_id().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            PrAccesoBaseDatos db = new PrAccesoBaseDatos();
            try {
                db.pr_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            PrAccesoBaseDatos db = new PrAccesoBaseDatos();
            try {
                db.pr_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

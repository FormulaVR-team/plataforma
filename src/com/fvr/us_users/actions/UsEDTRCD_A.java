...
public class UsEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        key = (UsBean)request.getAttribute("key_Us");
        if ( key == null) key = new UsBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getUs_user_id() == null || key.getUs_user_id().trim().length() < 1
                ) {
		key.setUs_user_id( pantalla.getUs_user_id() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("users: modificar");
        ...
        if (  !( 
		   key.getUs_user_id() == null || key.getUs_user_id().trim().length() < 1
            ...
            try {
                registro = db.us_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            UsAccesoBaseDatos db = new UsAccesoBaseDatos();
            try {
                db.us_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            try {
            	...
            	// El que viene de pantalla está degradado:
            	registro.setUs_avatar( Subrutinas.getUsFromId(dataBase, registro).getUs_avatar() );
            	
                db.us_chgObj( dataBase, registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

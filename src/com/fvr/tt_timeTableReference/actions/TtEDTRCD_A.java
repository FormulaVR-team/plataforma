...
public class TtEDTRCD_A extends Action {
...
//            errores.add("error", new ActionMessage( "errors.detail", "'" + usuario + "' no autorizado a '" + proceso + "'." ));
    ...
    private String cargarPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        ///////////////////////////////////////////
        TtBean key = null;
        TtBean registro = null;
        ////////////////////////////////
        // Recupera la clave del 'request' (desde los "parámetros"...) por si estuviera:
        key = (TtBean)request.getAttribute("key_Tt");
        if ( key == null) key = new TtBean();
        // Si no estaba en los "parámetros" la toma desde la pantalla: (es una reejecución... filtro, o lo que sea)
        if (
		   key.getTt_location_id() == null || key.getTt_location_id().trim().length() < 1
		|| key.getTt_day_type() == null || key.getTt_day_type().trim().length() < 1
		|| key.getTt_start_time() == null || key.getTt_start_time().trim().length() < 1
                ) {
		key.setTt_location_id( pantalla.getTt_location_id() );
		key.setTt_day_type( pantalla.getTt_day_type() );
		key.setTt_start_time( pantalla.getTt_start_time() );
            }
        ///////////////////////////////////////////
        ...
        cfg.setTituloPantalla("timeTableReference: modificar");
		...
		|| key.getTt_start_time() == null || key.getTt_start_time().trim().length() < 1
            ...
            try {
                registro = db.tt_getRcd( new Subrutinas().getBDConexion(request), key );
    ...
    private String chkPantalla(HttpServletRequest request, ActionForm  form) {
        ...
        // Claves obligatorias:
        if (
		   pantalla.getTt_location_id() == null || pantalla.getTt_location_id().trim().length() < 1
		|| pantalla.getTt_day_type() == null || pantalla.getTt_day_type().trim().length() < 1
		|| pantalla.getTt_start_time() == null || pantalla.getTt_start_time().trim().length() < 1
                ) {
            resultado = "NOVALE";
    ...
    private String dltRcd(HttpServletRequest request, ActionForm  form) {
        ...
        if (pantalla != null ) {
            ...
            TtAccesoBaseDatos db = new TtAccesoBaseDatos();
            try {
                db.tt_dltObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));
    ...
    private String chgRcd(HttpServletRequest request, ActionForm  form ) {
        ...
        if (pantalla != null ) {
            ...
            TtAccesoBaseDatos db = new TtAccesoBaseDatos();
            try {
                db.tt_chgObj( new Subrutinas().getBDConexion(request), registro );
            } catch (StExcepcion ex) {
                resultado = "ERROR";
                ActionMessages errores = new ActionMessages();
                errores.add("error", new ActionMessage( "errors.detail", ex.getMessage() ));

...
public class TtBeanFiltro extends StBean {
	...
	public String tt_duration_minutes; // duration_minutes
    ...
    public TtBeanFiltro(Object nulo) { super(); }
	...
	this.setTt_duration_minutes( "" ); // duration_minutes
	this.setTt_isBlocked( "" ); // isBlocked
	this.setTt_json( "" ); // json
    } 

    public TtBeanFiltro coalesce(TtBeanFiltro pri, TtBeanFiltro sec) {
        TtBeanFiltro r = new TtBeanFiltro(null);
        if ( pri!=null && sec!=null ) {
		r.setTt_sincro( (pri.getTt_sincro()==null)?sec.getTt_sincro():pri.getTt_sincro() );	// sincro
		r.setTt_mark( (pri.getTt_mark()==null)?sec.getTt_mark():pri.getTt_mark() );	// mark
		r.setTt_is_deleted( (pri.getTt_is_deleted()==null)?sec.getTt_is_deleted():pri.getTt_is_deleted() );	// is_deleted
		r.setTt_author( (pri.getTt_author()==null)?sec.getTt_author():pri.getTt_author() );	// author
		r.setTt_location_id( (pri.getTt_location_id()==null)?sec.getTt_location_id():pri.getTt_location_id() );	// location_id
		r.setTt_day_type( (pri.getTt_day_type()==null)?sec.getTt_day_type():pri.getTt_day_type() );	// day_type
		r.setTt_start_time( (pri.getTt_start_time()==null)?sec.getTt_start_time():pri.getTt_start_time() );	// start_time
		r.setTt_duration_minutes( (pri.getTt_duration_minutes()==null)?sec.getTt_duration_minutes():pri.getTt_duration_minutes() );	// duration_minutes
		r.setTt_isBlocked( (pri.getTt_isBlocked()==null)?sec.getTt_isBlocked():pri.getTt_isBlocked() );	// isBlocked
		r.setTt_json( (pri.getTt_json()==null)?sec.getTt_json():pri.getTt_json() );	// json
        }
        return r;
    }
    
    public void copyTo(TtBeanFiltro Destino) {
    ...
    public void copyFrom(TtBeanFiltro Origen) {
	...
	public String serializar() {
		...
		out.append( _K.sepFld ); out.append( this.getTt_duration_minutes()==null?"":this.getTt_duration_minutes() ); // duration_minutes
	...
	public void deserializar(String in) {
		...
		if ( in != null && in.length() > 0 ) {
			...
			try { this.setTt_duration_minutes( trozos[7] ); } catch (Exception e) {;} // duration_minutes
	...
	public String getTt_duration_minutes() {return tt_duration_minutes;}
	/** Set duration_minutes*/
	public void setTt_duration_minutes(String tt_duration_minutes) {this.tt_duration_minutes = tt_duration_minutes;}

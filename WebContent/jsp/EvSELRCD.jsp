...
<html>
                ...
                for (i=0;i<parms.length;i++) {
                    ...
                    if ( parm.length == 2 ) {
                        ...
                        if ( parm[0].toLowerCase() == 'form' )    
                            window.document.forms['EvRCD_AF'].elements["retFormulario"].value = parm[1];
                        if ( parm[0].toLowerCase() == 'element' ) 
                            window.document.forms['EvRCD_AF'].elements["retElemento"].value   = parm[1];
                        if ( parm[0].toLowerCase() == 'value' ) 
                            window.document.forms['EvRCD_AF'].elements["valorInicial"].value   = parm[1];
                    }
                }
                // Si es primera ejecución...(Si se le llamó con parámetros...)
                if ( parms.length > 1 ) {
                    // Inicializo primer restrictor con 'valor inicial' recibido:
                    window.document.forms['EvRCD_AF'].elements["ev_filtro.ev_sincro"].value   =                
                    window.document.forms['EvRCD_AF'].elements["valorInicial"].value;
                }
            }
            function setRetorno( retValor ) {
                var retFormulario = window.document.forms['EvRCD_AF'].elements["retFormulario"].value;
                var retElemento   = window.document.forms['EvRCD_AF'].elements["retElemento"].value;
    ...
    <body onload="javascript:persistePeticionario();" style="background-color: lightyellow; opacity:0.2;" onload="$('body').fadeTo('slow',1);">
        <html:form action="/EvSELRCD_A.do" style="margin: 10px 10px;">
            <table>
                <thead>
                    <tr>
                        <td colspan="100">
...
<td class="tbceldadetalle"><bean:write name="grid" property="ev_sincro"/><br/></td>   <!-- sincro -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_mark"/><br/></td>   <!-- mark -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_is_deleted"/><br/></td>   <!-- is_deleted -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_author"/><br/></td>   <!-- author -->
<td class="tbceldadetalle" onmouseover="onTdOver(this);" onmouseout="onTdOut(this);" onclick="javascript:setRetorno( '<bean:write name="grid" property="ev_event_id"/>' );"><bean:write name="grid" property="ev_event_id"/><br/></td>   <!-- event_id -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_location_id"/><br/></td>   <!-- location_id -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_LO_name"/><br/></td>   <!-- LO_name -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_name"/><br/></td>   <!-- name -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="ev_max_inscriptions"/><br/></td>   <!-- max_inscriptions -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="ev_amount"/><br/></td>   <!-- amount -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_currency"/><br/></td>   <!-- currency -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_deadline"/><br/></td>   <!-- deadline -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_comment"/><br/></td>   <!-- comment -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_date1"/><br/></td>   <!-- date1 -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_date2"/><br/></td>   <!-- date2 -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_date3"/><br/></td>   <!-- date3 -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_date4"/><br/></td>   <!-- date4 -->
<td class="tbceldadetalle"><bean:write name="grid" property="ev_json"/><br/></td>   <!-- json -->
                    </tr>
                </logic:iterate>
            </table>

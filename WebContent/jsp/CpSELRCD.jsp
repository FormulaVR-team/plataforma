...
<html>
                ...
                for (i=0;i<parms.length;i++) {
                    ...
                    if ( parm.length == 2 ) {
                        ...
                        if ( parm[0].toLowerCase() == 'form' )    
                            window.document.forms['CpRCD_AF'].elements["retFormulario"].value = parm[1];
                        if ( parm[0].toLowerCase() == 'element' ) 
                            window.document.forms['CpRCD_AF'].elements["retElemento"].value   = parm[1];
                        if ( parm[0].toLowerCase() == 'value' ) 
                            window.document.forms['CpRCD_AF'].elements["valorInicial"].value   = parm[1];
                    }
                }
                // Si es primera ejecución...(Si se le llamó con parámetros...)
                if ( parms.length > 1 ) {
                    // Inicializo primer restrictor con 'valor inicial' recibido:
                    window.document.forms['CpRCD_AF'].elements["cp_filtro.cp_sincro"].value   =                
                    window.document.forms['CpRCD_AF'].elements["valorInicial"].value;
                }
            }
            function setRetorno( retValor ) {
                var retFormulario = window.document.forms['CpRCD_AF'].elements["retFormulario"].value;
                var retElemento   = window.document.forms['CpRCD_AF'].elements["retElemento"].value;
    ...
    <body onload="javascript:persistePeticionario();" style="background-color: lightyellow; opacity:0.2;" onload="$('body').fadeTo('slow',1);">
        <html:form action="/CpSELRCD_A.do" style="margin: 10px 10px;">
            <table>
                <thead>
                    <tr>
                        <td colspan="100">
                            <fieldset style="text-align: left; height: 50px;">
                                <div style="font-size: 20px;">&nbsp;cockpits</div>
                                &nbsp;
					            <div class="pull-left">
									<div class="btn-group">
...
<td class="tbceldadetalle"><bean:write name="grid" property="cp_sincro"/><br/></td>   <!-- sincro -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_mark"/><br/></td>   <!-- mark -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_is_deleted"/><br/></td>   <!-- is_deleted -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_author"/><br/></td>   <!-- author -->
<td class="tbceldadetalle" onmouseover="onTdOver(this);" onmouseout="onTdOut(this);" onclick="javascript:setRetorno( '<bean:write name="grid" property="cp_location_id"/>' );"><bean:write name="grid" property="cp_location_id"/><br/></td>   <!-- location_id -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_LO_name"/><br/></td>   <!-- LO_name -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_LO_address"/><br/></td>   <!-- LO_address -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_LO_city"/><br/></td>   <!-- LO_city -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_LO_postal_code"/><br/></td>   <!-- LO_postal_code -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="cp_LO_lat"/><br/></td>   <!-- LO_lat -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="cp_LO_lon"/><br/></td>   <!-- LO_lon -->
<td class="tbceldadetalle" onmouseover="onTdOver(this);" onmouseout="onTdOut(this);" onclick="javascript:setRetorno( '<bean:write name="grid" property="cp_cockpit_id"/>' );"><bean:write name="grid" property="cp_cockpit_id"/><br/></td>   <!-- cockpit_id -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_serial_number"/><br/></td>   <!-- serial_number -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_name"/><br/></td>   <!-- name -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_isBlocked"/><br/></td>   <!-- isBlocked -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="cp_asignation_order"/><br/></td>   <!-- asignation_order -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_install_date"/><br/></td>   <!-- install_date -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_reset_date_used"/><br/></td>   <!-- reset_date_used -->
<td class="tbceldadetalle" style="text-align: right;"><bean:write name="grid" property="cp_hours_used"/><br/></td>   <!-- hours_used -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_note"/><br/></td>   <!-- note -->
<td class="tbceldadetalle"><bean:write name="grid" property="cp_comment"/><br/></td>   <!-- comment -->

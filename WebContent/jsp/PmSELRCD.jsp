...
<html>
                ...
                for (i=0;i<parms.length;i++) {
                    ...
                    if ( parm.length == 2 ) {
                        ...
                        if ( parm[0].toLowerCase() == 'form' )    
                            window.document.forms['PmRCD_AF'].elements["retFormulario"].value = parm[1];
                        if ( parm[0].toLowerCase() == 'element' ) 
                            window.document.forms['PmRCD_AF'].elements["retElemento"].value   = parm[1];
                        if ( parm[0].toLowerCase() == 'value' ) 
                            window.document.forms['PmRCD_AF'].elements["valorInicial"].value   = parm[1];
                    }
                }
                // Si es primera ejecución...(Si se le llamó con parámetros...)
                if ( parms.length > 1 ) {
                    // Inicializo primer restrictor con 'valor inicial' recibido:
                    window.document.forms['PmRCD_AF'].elements["pm_filtro.pm_sincro"].value   =                
                    window.document.forms['PmRCD_AF'].elements["valorInicial"].value;
                }
            }
            function setRetorno( retValor ) {
                var retFormulario = window.document.forms['PmRCD_AF'].elements["retFormulario"].value;
                var retElemento   = window.document.forms['PmRCD_AF'].elements["retElemento"].value;
    ...
    <body onload="javascript:persistePeticionario();" style="background-color: lightyellow; opacity:0.2;" onload="$('body').fadeTo('slow',1);">
        <html:form action="/PmSELRCD_A.do" style="margin: 10px 10px;">
            <table>
                <thead>
                    <tr>
                        <td colspan="100">
                            <fieldset style="text-align: left; height: 50px;">
                                <div style="font-size: 20px;">&nbsp;promosManuales</div>
                                &nbsp;
					            <div class="pull-left">
									<div class="btn-group">
...
<td class="tbceldadetalle"><bean:write name="grid" property="pm_sincro"/><br/></td>   <!-- sincro -->
<td class="tbceldadetalle"><bean:write name="grid" property="pm_mark"/><br/></td>   <!-- mark -->
<td class="tbceldadetalle"><bean:write name="grid" property="pm_is_deleted"/><br/></td>   <!-- is_deleted -->
<td class="tbceldadetalle"><bean:write name="grid" property="pm_author"/><br/></td>   <!-- author -->

	        ...
	        ,  function($http,$window) {
		        	...
		        	return {
		        		filtrar: function(actionForm) {
		        			/////////////////////////
		        			// Persistir su filtro:
		        			$window.sessionStorage.setItem("Ad_rsDSPFIL.ad_rs_filtro", angular.toJson( actionForm.ad_rs_filtro ));
		        			$window.sessionStorage.setItem("filasGrid", angular.toJson( actionForm.filasGrid ));
		        			/////////////////////////
			       			actionForm.opcionPantalla = "Filtrar";
			       			return $http({
					                url: './Ad_rsDSPFIL_A.do',
					                method: 'POST',
					                data: actionForm
		        	  ...
		        	  , add: function(actionForm){
		        			...
		        			return $http({
		        				url: './Ad_rsADDRCD_A.do',		// No es DSPFIL, es ADDRCD. ATENCIÓN !!!
		        				method: 'POST',
		        				data: actionForm
				            }).then ( function( response) { return response; });
		        		}
		        	  , chg: function(actionForm){
		        			...
		        			return $http({
		 		                url: './Ad_rsEDTRCD_A.do',		// No es DSPFIL, es EDTRCD. ATENCIÓN !!!
		 		                method: 'POST',
		 		                data: actionForm
				            }).then ( function( response) { return response; });
		        		}
		        	  ...
		        	  , executeReservation: function(actionForm,casoPagoEnEstablecimiento){
		        			actionForm.opcionPantalla = "executeReservation";
		        			
		        			if( casoPagoEnEstablecimiento != null ) {
		        				actionForm.opcionPantalla += "_" + casoPagoEnEstablecimiento;
		        			};
